...
`db:down` and `lane:clean` are deliberately outside R3. Both act on the shared world from
the main checkout rather than as a lane: `db:down` stops the shared containers, and
`lane:clean` walks every worktree and delegates `lane:down` to each one, which is where the
lane values are actually read.
