...
Three commands, translated directly to REST endpoints. `user_id` is sent on every
...
### `next-question` — `POST /api/sessions/:id/next`

```
body: { user_id: string, option_index: number }
resp: { session_id: string, question: Question | null, position: { position, total }, complete: boolean }
```

Records the answer to the *current* question in the session (via `evaluate` from
`@lang-tutor/core/domain`), advances the server's cursor, and returns the next question —
or `question: null, complete: true` if that was the 10th answer. Re-submitting an answer
to the question the session is currently on (rather than moving forward) is **idempotent**:
it returns the same recorded result again without changing state, rather than erroring —
this covers double-taps and retried requests. This includes a retried call *after*
completion: if the session is already `complete`, `next-question` returns the same
`{ question: null, complete: true }` response again without re-triggering the completion
log line a second time.
...
stdout. This happens here — not deferred to `get-results` — because the client may never
call `get-results` (e.g. the app is closed right after the last question).

### `get-results` — `GET /api/sessions/:id/results?user_id=...`

```
resp: { score: { correct: number, total: number }, missed_questions: MissedQuestion[] }
```

Only valid after `complete: true` was returned from `next-question`. Reading the results
evicts the session from the server's in-memory store — sessions don't accumulate in memory
for the life of the process, since nothing else ever cleans them up. Calling it before the
session is complete → `409`; the client never does this in the normal flow (it only calls
`get-results` in response to `complete: true`).

### Errors

- Unknown `session_id` (never created, or the server restarted mid-session) → `404`.
  The client shows a simple "something went wrong, let's start over" screen and routes
  back to Home. No retry/reconnect logic — restarting the quiz is an acceptable outcome
  for a phase with no persistence.
...
└── src/
    ...
    ├── routes/
    │   ├── sessions.ts       the 3 route handlers
    │   └── schemas.ts        Zod schemas for the two request bodies
    ├── store/
    │   └── sessionStore.ts   in-memory Map<session_id, SessionRecord>; create/get/complete/evict
...
**New:** `src/api/client.ts` — three functions (`createSession`, `nextQuestion`,
`getResults`) wrapping `fetch` against a configurable base URL.

**`useSession` hook (`src/hooks/useSession.tsx`):** rewritten internally to drive local
React state from the three API calls instead of local pure functions, but its **public
...
2. **Option tap:** compute `is_correct` locally from the tapped option against
   ...
   background, call `nextQuestion` to register the answer server-side and fetch what's
   next.
3. **Continue tap:** swap the displayed question to the already-fetched next one (again,
   no network wait) — unless the fetched response was `complete: true`, in which case
   call `getResults` and navigate to the Results screen.
...
- `apps/server/src/session.ts` — unit tests analogous to phase 1's `session.test.ts`:
  create, answer-and-advance, completion, idempotent re-answer of the current question.
- `apps/server/src/store/sessionStore.ts` — unit tests for create/get/complete/evict.
...
│   │   └── src/
│   │       ├── api/
│   │       │   ├── client.ts          createSession, nextQuestion, getResults
│   │       │   └── client.test.ts
