...
plan and changes nothing until given `npm run lane:clean -- --yes` — the bare `--` is
required, or npm keeps the flag and the script dry-runs again. It skips three kinds of
worktree: locked
...
Giving up on a lane is a different command, on purpose. `npm run lane:abandon -- <name>`
destroys exactly what `lane:clean` protects — uncommitted work and unmerged commits — so it
takes one named lane, never `--all`, refuses a locked one, and prints what it will delete
before `--yes` makes it act. A safe housekeeping command and a destructive one must not
share a name: `lane:clean --force` would put every unmerged branch one mistyped flag away.
